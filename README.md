...
### 1. `packages/viator-vscode/`
一个轻量级的 VS Code 插件。
- **绝对无痕：** 用户的批注数据完全与项目代码隔离，你在编辑器里的所有操作，都不会在当前项目树中留下任何可见的文件或数据残留。
- **原生高亮：** 使用 VS Code Decorations API，在划词处渲染优雅的淡黄色高亮及悬浮气泡，绝不修改源文件字符。
- **幽灵同步：** 监听全局数据库，一旦 AI 完成修改，屏幕上的高亮会瞬间消失。

### 2. `packages/viator-mcp-server/`
...
### 3. `packages/viator-skill/`
轻量级的极简 AI 指令流 (Agent Skill)。
